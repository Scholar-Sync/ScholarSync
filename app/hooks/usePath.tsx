...
const generateTabShapePath = (position, adjustedHeight) => {
    ...
    const tabX = adjustedWidth * position; // Corrected the variable name
    ...
    return `${tab}`; // Corrected to use backticks for template literal
};
...
const usePath = () => {
    ...
    const curvedPaths = useMemo(() => {
        return Array.from({length: NUM_TABS}, (_, index) => {
            ...
            return parse(`${tabShapePath}`); // Ensure parse function is suitable here
        });
    }, [adjustedHeight]);
